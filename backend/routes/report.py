...
from database.mongodb import exams_collection
...
def get_report(report_id):
    ...
    if get_jwt().get("role") == "student" and report.get("studentEmail") != get_jwt_identity():
        return jsonify(success=False, message="Report not found"), 404
    if get_jwt().get("role") == "faculty":
        try:
            from bson import ObjectId
            exam = exams_collection().find_one({"_id": ObjectId(report.get("examId")), "facultyId": get_jwt_identity()})
        except Exception:
            exam = None
        if not exam:
            return jsonify(success=False, message="Report not found"), 404
